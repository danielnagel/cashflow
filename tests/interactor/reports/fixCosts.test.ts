    ...
    describe("Test categorized fix costs", () => {
        ...
        describe("Test falsy parameters", () => {
            test("Return null, if transactions array is empty", () => {
                ...
                expect(generateCategorizedFixCosts([], options, { allowedLogLevel: "none" })).toStrictEqual({ source: "fixCosts.ts", message: "There are no transactions." });
            });
            ...
            test("Return null, if categories array is empty", () => {
                ...
                expect(generateCategorizedFixCosts(transactions, options, { allowedLogLevel: "none" })).toStrictEqual({ source: "fixCosts.ts", message: "There are no categories." });
            });

            test("Return null, if no transaction can be categorized", () => {
                const options: CategorizeOptions = { categories: [{ name: "a", samples: [{ initiator: "b" }, { initiator: "c" }] }] };
                expect(generateCategorizedFixCosts(transactions, options, { allowedLogLevel: "none" })).toStrictEqual({ source: "fixCosts.ts", message: "Couldn't match any categories." });
            });
        });
        ...
        describe("Test categorized fix costs to match multiple categories to a specific date", () => {
            ...
            test("Generate categorized fix costs as expected", () => {
                ...
                expect(generateCategorizedFixCosts(transactions, options, { allowedLogLevel: "none" })).toStrictEqual(expected);
            });
        });
    });
