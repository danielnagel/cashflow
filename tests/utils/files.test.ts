import { fileExists, loadFile, loadFileNamesFromDirectory, isFile } from "../../src/utils/files";
...
describe("Test utils/files", () => {
    ...
    describe("Load files from directory", () => {
        test("Return emptyArray if directory does not exist", () => {
            expect(loadFileNamesFromDirectory(__dirname + "/notexisting")).toHaveLength(0);
        });

        test("Return emptyArray if directory is empty", () => {
            expect(loadFileNamesFromDirectory(__dirname + "/empty")).toHaveLength(0);
        });

        test("Load file names from directory", () => {
            const fileNames = loadFileNamesFromDirectory(__dirname + "/samples");
        ...
        test("Load file names with specific file extension from directory", () => {
            const fileNames = loadFileNamesFromDirectory(__dirname + "/samples", "csv");
