...
export const logToConsole = (options: LogOptions): void => {
    if(isLogLevelAllowed(options.level, options.allowedLogLevel))
        console.log(generateLogMessage(options.message, options.level, options.dateTimeFormat))
}

const generateLogMessage = (message: string | ApplicationError, level = "error", dateTimeFormat = "dd.MM.yyyy HH:mm:ss"): string => {
...
const isLogLevelAllowed = (level: string | undefined, allowedLogLevel: string | undefined): boolean => {
